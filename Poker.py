...
class PokerHand(Cards.Hand):
    ...
    def __gt__(self, other):
        if self.best_hand[0] == other.best_hand[0]:
            # case if the hands are the same (e.g. straight vs straight)
            for i in range(len(self.best_hand)):
                if self.best_hand[1][i] < other.best_hand[1][i] or self.best_hand[1][i] > other.best_hand[1][i]:
                    return self.best_hand[1][i] > other.best_hand[1][i]
            # if the whole for loop is executed the hands are exactly the same
            return False
    ...
    def description(self, a_num, cards):
        ...
        Parameters:
            ...
            - cards: a list of at least 2 cards
        ...
        Return:
             A string.
        """
        assert len(cards) >= 2, f"cards' length is {len(cards)}, must be at least 2!"
        text = ""
        if a_num == 10:
            text = "Royal flush"
        elif a_num == 9:
            ...
            if len(cards) >= 4:
                ...
                text = "Trips " + cards[0].symbol + "'s."
        elif a_num == 3:
            if len(cards) > 4:
                text = ("Two pairs, " + cards[0].symbol + "'s and " + cards[2].symbol
                        + "'s, kicker " + cards[-1].symbol + ".")
            else:
                text = ("Two pairs, " + cards[0].symbol + "'s and " + cards[2].symbol+ "'s.")
        elif a_num == 2:
            if len(cards) > 2:
                text = "A pair of " + cards[0].symbol + "'s, kicker " + cards[2].symbol + "."
            else:
                text = "A pair of " + cards[0].symbol + "'s."
        elif a_num == 1:
            text = "High card " + cards[0].symbol + ", kicker " + cards[1].symbol + "."
        return text
    ...
    def high_card(self):
        """
        Returns:
            - if at least 2 cards: the highest cards.
            - otherwise: empty list
        """
        high_card = []
        if len(self.cards) >= 2:
            while len(high_card) < 5 and len(high_card) < len(self.cards):
                high_card.append(self.cards[len(high_card)])
        return high_card
    ...
    def is_straight(self, some_cards=None):
        ...
        if len(some_cards) >= 5:
            some_cards.sort(key=lambda c: c.poker_value, reverse=True)
            current = []
    ...
    def is_two_pairs(self):
        ...
        if len(self.cards) >= 4:
            ...
            if two_pairs:
                for card in self.cards:
                    if (card > two_pairs[0] or card < two_pairs[0]) and (card > two_pairs[-1] or card < two_pairs[-1]):
...
class PokerPlayer:
    ...
    Attributes
    __________
    Hole Cards : Hand object from Cards
        the player's private cards
    Poker Hand : Poker_Hand object from Poker
        the actual 5-Card poker hand, formed by hole cards + community cards
    Stack : Integer
        the player's available stack
    """

    def __init__(self, hole_cards=None):
        self.HoleCards = Cards.Hand()
        self.PokerHand = PokerHand()
        if hole_cards:
            for card in hole_cards:
                self.HoleCards.get_card(card)
        self.Stack = 0

    def __str__(self):
        return str(self.HoleCards) + "\n" + str(self.PokerHand)

    def get_card(self, a_card):
        self.HoleCards.get_card(a_card)

    def eval_poker_hand(self, community_cards=None):
        self.PokerHand = PokerHand(self.HoleCards.cards)
        if community_cards:
            for card in community_cards:
                self.PokerHand.get_card(card)

    def get_poker_hand(self):
        return self.PokerHand

    def give_card(self, a_card):
        self.HoleCards.give_card(a_card)
